...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  useEffect(() => {
    ...
    if (style === 'goober' && backgroundColor) {
      const getBackgroundColorRgba = (bgColor: string, opacity: number): string => {
        ...
        if (hex.length !== 6 || !/^[0-9a-fA-F]+$/.test(hex)) return `rgba(20, 20, 20, ${opacity})`;
  ...
  const parseHex = (bgColor: string): [number, number, number] | null => {
    let hex = bgColor.replace('#', '');
  ...
  const getTextShadowStyle = (bgColor: string): React.CSSProperties => {
    ...
    if (!rgb) return { textShadow: '0 2px 6px rgba(20, 20, 20, 0.5), 0 1px 3px rgba(20, 20, 20, 0.3)' };
    return { textShadow: `0 2px 6px rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, 0.5), 0 1px 3px rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, 0.3)` };
  };

  const getShadowColorRgba = (bgColor: string): string => {
    const rgb = parseHex(bgColor);
    if (!rgb) return 'rgba(20, 20, 20, 0.3)';
  ...
  const renderMobileToggle = () => {
    return (
      ...
      >
        <div className="navbar__hamburger" aria-hidden="true">
          <span></span>
          <span></span>
          <span></span>
        </div>
...
const MobileMenuPortal: React.FC<{
  ...
  const menuContent = (
    <>
      <div 
        className={cn('navbar__mobile-menu-backdrop', visibleClass)}
        onClick={onClose}
        aria-hidden="true"
      />
      <div 
